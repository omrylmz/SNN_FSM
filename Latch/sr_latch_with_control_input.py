...
def SR_Latch():
    with nengo.Network() as SRL:
        ...
        SRL.Enable = nengo.Ensemble(200, dimensions=1, radius=1)
        SRL.NAND0 = NAND()
        SRL.NAND1 = NAND()
        SRL.NAND2 = NAND()
        SRL.NAND3 = NAND()
        ...
        nengo.Connection(SRL.Enable, SRL.NAND0.I[1])
        nengo.Connection(SRL.R, SRL.NAND1.I[0])
        nengo.Connection(SRL.Enable, SRL.NAND1.I[1])
        nengo.Connection(SRL.NAND0.O, SRL.NAND2.I[0])
        nengo.Connection(SRL.NAND3.O, SRL.NAND2.I[1])
        nengo.Connection(SRL.NAND2.O, SRL.NAND3.I[0])
        nengo.Connection(SRL.NAND1.O, SRL.NAND3.I[1])

        SRL.Q = nengo.Ensemble(200, dimensions=1, radius=1)
        SRL.Q_ = nengo.Ensemble(200, dimensions=1, radius=1)
        nengo.Connection(SRL.NAND2.O, SRL.Q)
        nengo.Connection(SRL.NAND3.O, SRL.Q_)
    return SRL

model = nengo.Network(label='SR Latch with control input')
with model:
    SRl = SR_Latch()
    stim_S = nengo.Node(-1)
    stim_R = nengo.Node(-1)
    ...
    nengo.Connection(stim_Enable, SRl.Enable)
